...
$box_smarty->assign('tpl_path','templates/'.CURRENT_TEMPLATE.'/');
  ...
  $orders_contents .='<a href="' . xtc_href_link_admin(FILENAME_ORDERS, 'selected_box=customers&amp;status=0', 'SSL') . '" target="_blank" title="'.TEXT_VALIDATING.'">' . TEXT_VALIDATING . '</a>: ' . $orders_status_validating . '<br />';
  ...
  while ($orders_status = xtc_db_fetch_array($orders_status_query)) {
    ...
    $orders_contents .= '<a href="' . xtc_href_link_admin(FILENAME_ORDERS, 'selected_box=customers&amp;status=' . $orders_status['orders_status_id'], 'SSL') . '" target="_blank" title="'.$orders_status['orders_status_name'].'">' . $orders_status['orders_status_name'] . '</a>: ' . $orders_pending['count'] . '<br />';
  }
  ...
  $admin_image = '<a href="' . xtc_href_link_admin(FILENAME_START,'', 'SSL').'" target="_blank">'.xtc_image_button('button_admin.gif', IMAGE_BUTTON_ADMIN).'</a>';
   if ($product->isProduct()) {
    $admin_link='<a href="' . xtc_href_link_admin(FILENAME_EDIT_PRODUCTS, 'cPath=' . $cPath . '&amp;pID=' . $product->data['products_id']) . '&amp;action=new_product' . '" onclick="window.open(this.href); return false;" target="_blank">' . xtc_image_button('edit_product.gif', IMAGE_BUTTON_PRODUCT_EDIT) . '</a>';
  }
