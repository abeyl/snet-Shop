...
/* -----------------------------------------------------------------------------------------
  ...
  $box_content='';
  $box_content=xtc_draw_form('currencies', xtc_href_link(basename($PHP_SELF), '', $request_type, false), 'get').xtc_draw_pull_down_menu('currency', $currencies_array, $_SESSION['currency'], 'onChange="this.form.submit();" style="width: 80%"') . $hidden_get_variables . xtc_hide_session_id().'</form>';
