...
E013 - 14.07.2010 - AB - Bankleitzahlen
*/

/* -----------------------------------------------------------------------------------------
   $Id: banktransfer_validation.php 2010-01-05 14:40:57 cr $   

   XT-Commerce - community made shopping

   Copyright (c) 2010 Christian Rothe
   -----------------------------------------------------------------------------------------
   based on: 
...
//      $Mark73 = $this->Method06($AccountNo, 'A98765432', TRUE, 10, 11);
  ...
  /* --- Added FrankM 20070822 ---
     --- Changed FrankM 20090206 --- */
  function MarkC6($AccountNo) {
    $AccountNo = $this->ExpandAccount($AccountNo);
    // 3, 4, 5, 6, 8 an erster Stelle von links = falsch.
    if ((substr($AccountNo,0,1) == "3") or (substr($AccountNo,0,1) == "4") or (substr($AccountNo,0,1) == "5") or (substr($AccountNo,0,1) == "6") or (substr($AccountNo,0,1) == "8")) {
      $markC6 = 1;
    // 0, 1, 2, 7, 9 an erster Stelle von links = mit Methode 00 pruefen.
    } else {
      $Help = '5499570' . $AccountNo;
