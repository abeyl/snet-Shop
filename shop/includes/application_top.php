...
// ... A002 - begin
//
require_once (DIR_FS_INC.'xtc_check_qty.inc.php');
//
// ... A002 - end
//
...
if (isset ($cPath_array)) {
	for ($i = 0, $n = sizeof($cPath_array); $i < $n; $i ++) {
		...
		if (xtc_db_num_rows($categories_query,true) > 0) {
			...
			// ... new
			//
			$breadcrumb->add(htmlspecialchars($categories['categories_name']), xtc_href_link(FILENAME_DEFAULT, xtc_category_link($cPath_array[$i], htmlspecialchars($categories['categories_name']))));
			//
			// ... B003 - end
			//
		} else {
