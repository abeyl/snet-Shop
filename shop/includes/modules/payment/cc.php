...
E011 - 01.07.2010 - AB - Fehlerbehandlung
E016 - 13.10.2010 - AB - CreditCard
*/
...
class cc {
	...
	function selection() {
		...
		for ($i = 1; $i < 13; $i ++) {
			...
			// ... new
			//
			$expires_month[] = array ('id' => sprintf('%02d', $i), 'text' => htmlentities(strftime('%B', mktime(0, 0, 0, $i, 1, 2000))));
			//
			// ... E016 - end
			//
		}
		...
		for ($i = 1; $i < 13; $i ++) {
			...
			// ... new
			//
			$start_month[] = array ('id' => sprintf('%02d', $i), 'text' => htmlentities(strftime('%B', mktime(0, 0, 0, $i, 1, 2000))));
			//
			// ... E016 - end
			//
		}
		...
		//
		if($_GET['cc_owner']) {
			$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_OWNER, 'field' => xtc_draw_input_field('cc_owner', $_GET['cc_owner']))));
		} else {
			$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_OWNER, 'field' => xtc_draw_input_field('cc_owner', $order->billing['firstname'].' '.$order->billing['lastname']))));
		}
		//
		...
		// ... new
		//
		$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_NUMBER, 'field' => xtc_draw_input_field('cc_number',$_GET['cc_number']))));
		//
		// ... E016 - end
		//
		...
		if (strtolower(USE_CC_START) == 'true') {
			...
			// ... new
			//
			$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_START, 'field' => xtc_draw_pull_down_menu('cc_start_month', $start_month, $_GET['cc_start_month']).'&nbsp;'.xtc_draw_pull_down_menu('cc_start_year', $start_year, $_GET['cc_start_year']))));
			//
			// ... E016 - end
			//
		}
		...
		// ... new
		//
		$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_EXPIRES, 'field' => xtc_draw_pull_down_menu('cc_expires_month', $expires_month, $_GET['cc_expires_month']).'&nbsp;'.xtc_draw_pull_down_menu('cc_expires_year', $expires_year, $_GET['cc_expires_year']))));
		//
		// ... E016 - end
		//
		...
		if (strtolower(USE_CC_CVV) == 'true') {
			...
			// ... new
			//
			$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_CVV.' '.'<a href="javascript:popupWindow(\''.xtc_href_link(FILENAME_POPUP_CVV, '', 'SSL').'\')">'.MODULE_PAYMENT_CC_TEXT_CVV_LINK.'</a>', 'field' => xtc_draw_input_field('cc_cvv', $_GET['cc_cvv'], 'size=4 maxlength=4'))));
			//
			// ... E016 - end
			//
		}
		...
		if (strtolower(USE_CC_ISS) == 'true') {
			...
			// ... new
			//
			$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_ISSUE, 'field' => xtc_draw_input_field('cc_issue', $_GET['cc_issue'], 'size=2 maxlength=2'))));
			//
			// ... E016 - end
			//
		}
	...
	function pre_confirmation_check() {
		...
		if (($result == false) || ($result < 1)) {
			...
			// ... new
			//
			$payment_error_return = 'payment_error='.$this->code.'&error='.urlencode($error).'&cc_owner='.urlencode($_POST['cc_owner']).'&cc_number='.$_POST['cc_number'].'&cc_expires_month='.$_POST['cc_expires_month'].'&cc_expires_year='.$_POST['cc_expires_year'].'&cc_cvv='.$_POST['cc_cvv'].'&cc_issue='.$_POST['cc_issue'];
			if (strtolower(USE_CC_START) == 'true') {
				$payment_error_return .= '&cc_start_month='.$_POST['cc_start_month'].'&cc_start_year='.$_POST['cc_start_year'];
			}
			//
			// ... E016 - end
			//
	...
	function confirmation() {
		...
		if (strtolower(USE_CC_START) == 'true') {
			...
			// ... new
			//
			$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_START, 'field' => htmlentities(strftime('%B, %Y', mktime(0, 0, 0, $_POST['cc_start_month'], 1, $_POST['cc_start_year']))))));
			//
			// ... E016 - end
			//
		}
		...
		// ... new
		//
		$form_array = array_merge($form_array, array (array ('title' => MODULE_PAYMENT_CC_TEXT_CREDIT_CARD_EXPIRES, 'field' => htmlentities(strftime('%B, %Y', mktime(0, 0, 0, $_POST['cc_expires_month'], 1, '20'.$_POST['cc_expires_year']))))));
		//
		// ... E016 - end
		//
	...
	function get_error() {
		...
		// ... new
		//
		$error = array ('title' => MODULE_PAYMENT_CC_TEXT_ERROR, 'error' => $_GET['error']);
		//
		// ... E011 - end
		//
		return $error;
