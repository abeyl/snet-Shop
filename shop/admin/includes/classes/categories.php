...
class categories {
	...
	function insert_category($categories_data, $dest_category_id, $action = 'insert') {
		...
		$categories_status = xtc_db_prepare_input($categories_data['status']);
                ...
                // ... E018 - begin
                //
                $target_url = xtc_db_prepare_input($categories_data['target_url']);
                //
                // ... E018 - end
                //
		...
		//$sql_data_array = array ('sort_order' => $sort_order, 'categories_status' => $categories_status, 'products_sorting' => xtc_db_prepare_input($categories_data['products_sorting']), 'products_sorting2' => xtc_db_prepare_input($categories_data['products_sorting2']), 'categories_template' => xtc_db_prepare_input($categories_data['categories_template']), 'listing_template' => xtc_db_prepare_input($categories_data['listing_template']));
                ...
                // ... new
                //
                $sql_data_array = array ('sort_order' => $sort_order, 'categories_status' => $categories_status, 'products_sorting' => xtc_db_prepare_input($categories_data['products_sorting']), 'products_sorting2' => xtc_db_prepare_input($categories_data['products_sorting2']), 'categories_template' => xtc_db_prepare_input($categories_data['categories_template']), 'listing_template' => xtc_db_prepare_input($categories_data['listing_template']),'target_url' => $target_url);
                //
                // ... E018 - end
                //
	...
	*/
	...
	if (!(in_array($src_category_id, $_SESSION['copied']))) {
			...
			$sql_data_array = array ('parent_id'=>xtc_db_input($dest_category_id),
									...
									'sort_order'=>$ccopy_values['sort_order'],
                                                                        ...
                                                                        // ... E018 - begin
                                                                        //
                                                                        'target_url'=>$ccopy_values['target_url'],
                                                                        //
                                                                        // ... E018 - end
                                                                        //
	...
	function remove_product($product_id) {
		...
		for ($i = 0, $n = sizeof($customers_status_array); $i < $n; $i ++) {
                    ...
                    //
                    if (isset($customers_statuses_array[$i]['id'])) {
			xtc_db_query("delete from xtc_personal_offers_by_customers_status_".$customers_statuses_array[$i]['id']." where products_id = '".xtc_db_input($product_id)."'");
                    }
                    //
                    //
                    // ... E017 - end
			...
			if ($group_data[$col]['STATUS_ID'] != '') {
				...
				if ($action == 'insert') {
					...
					//xtc_db_query("DELETE FROM personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID']." WHERE products_id = '".$products_id."' AND quantity    = '1'");
                                        ...
                                        // ... new
                                        //
                                        xtc_db_query("DELETE FROM xtc_personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID']." WHERE products_id = '".$products_id."' AND quantity = '1'");
                                        //
                                        // ... E017 - end
                                        //
					...
					//xtc_db_perform("personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID'], $insert_array);
                                        ...
                                        // ... new
                                        //
					xtc_db_perform("xtc_personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID'], $insert_array);
                                        //
                                        // ... E017 - end
                                        //
					...
					//							                 AND quantity    = '1'");
                                        ...
                                        // ... new
                                        //
					xtc_db_query("UPDATE xtc_personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID']."
                                                        SET personal_offer = '".$personal_price."'
                                                        WHERE products_id = '".$products_id."'
                                                        AND quantity = '1'");
                                        //
				...
				if ($staffelpreis != '' && $quantity != '') {
					...
					if ($quantity <= 1)
						$quantity = 2;
                                        ...
                                        //
                                        $check_query = xtc_db_query("SELECT quantity
                                                                        FROM xtc_personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID']."
                                                                        WHERE products_id = '".$products_id."'
                                                                        AND quantity    = '".$quantity."'");
                                        //
                                        // ... E017 - end
					...
					if (xtc_db_num_rows($check_query) < 1) {
                                            ...
                                            // ... new
                                            //
                                            xtc_db_query("INSERT INTO xtc_personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID']."
                                                                SET price_id = '',
                                                                products_id = '".$products_id."',
                                                                quantity = '".$quantity."',
                                                                personal_offer = '".$staffelpreis."'");
	...
	function duplicate_product($src_products_id, $dest_categories_id) {
		...
		for ($col = 0, $n = sizeof($group_data); $col < $n +1; $col ++) {
			if ($group_data[$col]['STATUS_ID'] != '') {
                                ...
                                // ... old
                                //
				//$copy_query = xtc_db_query("SELECT quantity, personal_offer
                                //                                FROM personal_offers_by_customers_status_".$group_data[$col]['STATUS_ID']."
