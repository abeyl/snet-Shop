  ...
  switch ($_GET['action']) {
      ...
      for ($i=0; $i<sizeof($languages); $i++) {
        ...
        if ($_GET['action'] == 'insert') {
          if (!xtc_not_null($customers_status_id)) {
            ...
            // ... new
            //
            xtc_db_query("CREATE TABLE xtc_personal_offers_by_customers_status_" . $customers_status_id . " (price_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, products_id int NOT NULL, quantity int, personal_offer decimal(15,4))");
            xtc_db_query("ALTER TABLE xtc_products ADD `group_permission_" . $customers_status_id . "` TINYINT( 1 ) NOT NULL");
            xtc_db_query("ALTER TABLE xtc_categories ADD `group_permission_" . $customers_status_id . "` TINYINT( 1 ) NOT NULL");
            $products_query = xtc_db_query("SELECT price_id, products_id, quantity, personal_offer FROM xtc_personal_offers_by_customers_status_" . $customers_base_status ."");
            //
            // ... E017 - end
            //
            ...
            while($products = xtc_db_fetch_array($products_query)){
                ...
                // ... new
                //
                xtc_db_perform('xtc_personal_offers_by_customers_status_' . $customers_status_id, $product_data_array);
                //
                // ... E017 - end
                //
            }
    ...
    case 'deleteconfirm':
      ...
      // We want to drop the existing corresponding personal_offers table
        ...
        // ... new
        //
        xtc_db_query("DROP TABLE IF EXISTS xtc_personal_offers_by_customers_status_" . xtc_db_input($cID) . "");
        xtc_db_query("ALTER TABLE xtc_products DROP `group_permission_" . xtc_db_input($cID) . "`");
        xtc_db_query("ALTER TABLE xtc_categories DROP `group_permission_" . xtc_db_input($cID) . "`");
        //
        // ... E017 - end
        //
