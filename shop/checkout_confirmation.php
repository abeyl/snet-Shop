...
if (DISPLAY_CONDITIONS_ON_CHECKOUT == 'true') {
	if ($_POST['conditions'] == false) {
		...
		//xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $error, 'SSL', true, false));
		//
		// ... new
		//
		xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=1', 'SSL', true, false));
		//
		// ... E011 - end
		//
	}
...
if ((is_array($payment_modules->modules) && (sizeof($payment_modules->modules) > 1) && (!is_object($$_SESSION['payment'])) && (!isset ($_SESSION['credit_covers']))) || (is_object($$_SESSION['payment']) && ($$_SESSION['payment']->enabled == false))) {
	...
	// ... new
	//
	xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=2', 'SSL'));
	//
	// ... E011 - end
	//
}
